...
import com.customer.fivecanale.uiutil.barcodescanning.graphic.FrameMetadata;
import com.customer.fivecanale.uiutil.barcodescanning.graphic.GraphicOverlay;
import com.customer.fivecanale.uiutil.barcodescanning.graphic.VisionImageProcessor;
...
public class CameraSource {
  ...
  private Camera createCamera() throws IOException {
    ...
    //
    // Through trial and mError it appears that two free buffers, in addition to the two buffers
  ...
  private class FrameProcessingRunnable implements Runnable {
    ...
    /**
     * Sets the frame mData received from the camera. This adds the previous unused frame buffer (if
     * present) back to the camera, and keeps a pending reference to the frame mData for future use.
     */
    ...
    public void run() {
      ...
      while (true) {
        synchronized (lock) {
          ...
          // Hold onto the frame mData locally, so that we can use this for detection
          // below.  We need to clear pendingFrameData to ensure that this buffer isn't
          // recycled back to the camera before we are done using that mData.
